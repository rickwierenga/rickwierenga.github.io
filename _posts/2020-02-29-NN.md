...
subtitle: ML from the Fundamentals (part 4)
category: ml-fundamentals
tags:
  - machinelearning
---

We have seen multiple classification models. In this post you will build a classifier model to classify images of handwritten digits. This may sound rather complicated problem to solve (what is "the number 5"?), but using the power of machine learning we do not have to define each number; it will learn by itself. Along the way I will introduce you to the most powerful classifier yet: neural networks. Entering *deep* learning for the first time.
...
As I just mentioned, in this post we will classify handwritten digits. To do so, we use a dataset called MNIST [1]. It consists of 60000 28 by 28 gray scale images like the following:

![MNIST images](/assets/images/nn/mnist.png)

In computer vision, a subfield-ish of machine learning, each pixel represents a feature. These images are gray scale so we can use the raw value of the pixels, but other datasets might be in RGB format. If that's the case, each channel of a pixel will be a feature.
...
where $x$ is an input vector, being mapped to an output vector $\hat{y}$.

If you were to visualize the individual elements of both vectors, you get something like this:

![classifier](/assets/images/nn/classifier1.png)

Here is the definition for $h$ again: (see [softmax regression](/blog/ml-fundamentals/softmax.html)):
...
The most important thing to understand is that every feature $x_j$ is multiplied by a row $j$ in $\theta$ ($\theta_j$). Each feature $x_j$ impacts the probability of the entire input $x$ belonging to a class. If we visualized the "impacts" in the schematic, we would get this:
...
In the context of a classifier network, people call nodes such as $\hat{y}$ "neurons." The complete network is, therefore, considered a "neural network." For consistency I will also use those terms, but I will simply define "neuron" as "node." Keep in mind that the biological definition of neuron is something else entirely.

## Extending the network

Simple networks like these fail at large classification tasks because they have too few parameters. It results in the entire model being too simple and it will never be able to learn from more interesting datasets such as MNIST. However, logistic regression can still be used in this problem. In fact, the main intuition I will present in this post is that a neural network is just a chained list of logistic classifiers.

It turns out that the graph I presented before can be modified to include another layer, or multiple other layers, the so-called "hidden layers." These layers are placed between the original input and the original output layer. The reason they are called "hidden" is because we do not directly use their values; they simply exist to forward the weights through the network. Note that these layers also have a bias factor.
...
Once you realize that the values for any $\Theta$ yield deterministic activations in the entire neural network given some input, it is understandable that not every activation in a hidden layer is desired, even though they are not directly interpreted by anyone (in fact, interpreting the values of hidden layers is an active problem).
...
We will compute the other errors in a loop. A few things:
...
* We also skip the first node in each layer; it always needs to be 1.
...
This snippet uses the derivative of sigmoid:
...
Congratulations! You now know backpropagation. Fun note: because we are learning in multiple layers, we are doing *deep* learning. Just so you know.

### Backpropagation for individual neurons

The way backpropagation is usually taught is by presenting it as a method for finding the derivative for the cost function.  A great intuition on backpropagation from that perspective is written by Andrej Karpathy. I’ll let him explain it himself: [http://cs231n.github.io/optimization-2/](http://cs231n.github.io/optimization-2/).

### The training loop

While you have already seen this many times, here is the training loop for a neural network. It's a little different from the other ones because we are updating the weights in a list.
...
This model is actually more advanced than our model, because under the hood it uses some techniques I haven't touched on in this series.
